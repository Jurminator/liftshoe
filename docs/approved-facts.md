# Approved facts — Liftshoe

The only source for factual statements on the page. **No fact here → no claim on the page.**
Only the owner (Jurjen) approves changes; changes to this file always go to a human review.
Status: **confirmed** = may be used · **needs evidence** = may be used only after the owner confirms proof exists · **open** = not known; page shows nothing or a `[[TODO]]` placeholder.
...
## Images
Owner instruction, 2026-09-18: no product photos yet — "invent for now"; OpenArt may be used for key hero images.
Rule applied: invented visuals are **illustrations of the idea** (seeing over a crowd), in a clearly illustrative style, not photo-real pictures presented as the actual product. Replace with real photos when available.
